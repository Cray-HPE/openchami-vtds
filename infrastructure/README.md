...
1) The GitHub action uses SSH to make a new test directory on the test
   platform from which all operations will run.
...
1) The entrypoint to the container is the `run_test_deployment.sh`
   ...
   1) compose a vTDS core configuration file in the deployment directory
      using the `config.yaml` found here as a template (the
      `config-test_vtds.yaml` found here is used for testing the vTDS test
      infrastructure, and is only used by developers).
